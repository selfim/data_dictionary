...
class DataDict
{
    ...
    protected  function generateHtml()
    {
        ...
        return $header . '<h1 style="text-align:center;">' . $title . '</h1>'  .
            '<header><p style="text-align:center;margin:20px auto;">总共：' . count($tables) .'个数据表'."&nbsp".'生成时间：' . date('Y-m-d H:i:s'). '</p></header>'
            .$html. '</body></html>';
...
<body>';
        $data = $header . '<h1 style="text-align:center;">' . $title . '</h1>'  . '<p style="text-align:center;margin:20px auto;">生成时间：' . date('Y-m-d H:i:s') . '</p>'.
            '<p style="text-align:left;margin:20px auto;">总共：' . count($tables) . '个数据表</p>' . '<p style="text-align:center;margin:20px auto;">生成时间：' . date('Y-m-d H:i:s') .$html.
            '</body></html>';
        echo $data;
    }
...
//        header("Content-Disposition:attachment;filename={$this->database}数据字典.csv");
